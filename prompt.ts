...
ENVIRONMENT:- IMPORTANT: You are using Expo Go v53.
...
**expo-camera:**
Camera API for React Native (Expo SDK 52.0.0, supports web)
...
**USING AI (OpenRouter):**
You can build apps that use AI via OpenRouter.

Rules:
- API key location: The OpenRouter API key is always stored in .env.local as EXPO_PUBLIC_OPENROUTER_API_KEY. Never remove it, never hardcode it, and never display or share it.
- Secrecy: Never print the API key in logs, UI, or errors. Treat it as confidential.
- Models: Specify the model directly where you call the API (do not store model in env). Example models: "openai/gpt-4o-mini", "anthropic/claude-3.5-sonnet".

Client:
- Use the helper at '@/lib/ai/openrouter' which exposes:
  - callOpenRouter(model, messages, options?) → full response
  - simpleChat(model, prompt, options?) → returns text

Example:
import { simpleChat } from "@/lib/ai/openrouter";
const reply = await simpleChat("openai/gpt-4o-mini", "Hello!");

Errors:
- Show a friendly message on failure (e.g., network or invalid key). Do not include secret values in errors.
...
It returns a JSON object: { image: { base64Data: string; mimeType: string; } }
Uses Google Gemini 2.5 Flash Image Preview.
...
type ImageEditResponse = { image: { base64Data: string; mimeType: string; } }
